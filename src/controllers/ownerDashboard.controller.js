...
const getOwnedRestaurant = async (req) => {
  ...
  if (!restaurantId) throw new ApiError(400, "Restaurant setup not completed");
...
export const getDashboardStats = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(
    new ApiResponse(200, { totalOrders, totalRevenue, pendingOrders, servedOrders, menuCount, topItems }, "Dashboard stats")
  );
});
...
export const deleteMenuItem = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, { deletedId: req.params.itemId }, "Menu item deleted"));
});
...
export const addCategory = asyncHandler(async (req, res) => {
  ...
  const exists = restaurant.categories.some((c) => c.name.toLowerCase() === name.toLowerCase().trim());
  ...
  const newCat = { name: name.trim(), emoji: emoji.trim(), image, imagePublicId };
  restaurant.categories.push(newCat);
  await restaurant.save();
...
export const deleteCategory = asyncHandler(async (req, res) => {
  ...
  const cat = restaurant.categories.id(categoryId);
...
export const getChefs = asyncHandler(async (req, res) => {
  const restaurant = await getOwnedRestaurant(req);
  const chefs = await Chef.find({ restaurantId: restaurant._id }).select("-passwordHash -refreshToken");
...
export const createChef = asyncHandler(async (req, res) => {
  ...
  return res.status(201).json(
    new ApiResponse(201, { chefId: chef._id, name: chef.name, kitchenId: chef.kitchenId }, "Chef account created")
  );
});

export const updateChef = asyncHandler(async (req, res) => {
  const restaurant = await getOwnedRestaurant(req);
  const chef = await Chef.findOne({ _id: req.params.chefId, restaurantId: restaurant._id }).select("+passwordHash");
  if (!chef) throw new ApiError(404, "Chef not found");

  const { name, password, isActive } = req.body;
  if (name) chef.name = name.trim();
  ...
  await chef.save();
  return res.status(200).json(new ApiResponse(200, { chefId: chef._id, name: chef.name }, "Chef updated"));
});

export const deleteChef = asyncHandler(async (req, res) => {
  const restaurant = await getOwnedRestaurant(req);
  const chef = await Chef.findOneAndDelete({ _id: req.params.chefId, restaurantId: restaurant._id });
  if (!chef) throw new ApiError(404, "Chef not found");
  return res.status(200).json(new ApiResponse(200, { deletedId: req.params.chefId }, "Chef deleted"));
});
...
export const updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  // Emit socket event to all connected clients in this restaurant room
