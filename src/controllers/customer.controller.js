...
export const scanQr = asyncHandler(async (req, res) => {
  ...
  if (!req.body || typeof req.body !== "object") {
    throw new ApiError(
      400,
      "Request body missing — set Content-Type: application/json"
    );
  }
  ...
  logger.debug(
    `[scan] body keys received: ${Object.keys(req.body).join(", ") || "(empty)"}`
  );
  ...
  logger.debug(
    `[scan] decoded → rid:${payloadRestaurantId} table:${payloadTableNumber} legacy:${!!isLegacy}`
  );
  ...
  if (
    !isLegacy &&
    payloadTableNumber !== null &&
    table.tableNumber !== payloadTableNumber
  ) {
  ...
  logger.info(
    `[scan] OK — "${restaurant.name}" table ${table.tableNumber} | customer ${customer._id}`
  );
...
export const placeOrder = asyncHandler(async (req, res) => {
  ...
  if (customerId && sessionTag) {
    ...
    if (!customer) {
      throw new ApiError(
        401,
        "Invalid session — please rescan the table QR code"
      );
    }
  }

  const restaurant = await Restaurant.findById(restaurantId)
    .select("isPublished menu")
    .lean();
  ...
  for (const reqItem of items) {
    ...
    if (!menuItem)
      throw new ApiError(400, `Menu item ${reqItem.itemId} not found`);
  ...
  const subtotal = enrichedItems.reduce((s, i) => {
    const extra = (i.customizations || []).reduce(
      (cs, c) => cs + (c.price ?? 0),
      0
    );
...
export const getCustomerOrders = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, orders, "Customer orders fetched"));
});
